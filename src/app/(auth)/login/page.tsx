    ...
    <div className="grid min-h-screen lg:grid-cols-[1.08fr_.92fr]">
      <section className="relative hidden overflow-hidden border-r border-[var(--border)] bg-[var(--surface-soft)] p-12 lg:flex lg:flex-col">
        ...
        <div className="relative my-auto max-w-xl">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--surface)] px-3 py-1.5 text-xs text-[var(--text-soft)]"><Braces className="h-3.5 w-3.5 text-[var(--primary)]" /> Built for engineering knowledge</div>
          <h1 className="text-5xl font-semibold leading-[1.08] tracking-[-0.05em]">Keep the reason behind every change.</h1>
          <p className="mt-6 max-w-lg text-base leading-7 text-[var(--text-soft)]">Code snippets, technical documentation, discussions, and AI guidance in one calm workspace.</p>
        </div>
      ...
      <section className="flex items-center justify-center px-6 py-12 sm:px-10">
        <div className="w-full max-w-[420px]">
          ...
          <p className="mt-7 text-center text-sm text-[var(--text-soft)]">New to DevKnowledge? <Link href="/register" className="font-medium text-[var(--text)] underline decoration-[var(--border-strong)] underline-offset-4">Create account</Link></p>
        </div>
      </section>
    </div>
  );
